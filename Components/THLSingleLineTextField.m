...
@interface THLSingleLineTextField ()
...
@implementation THLSingleLineTextField
...
{
    return [self initWithFrame:frame labelHeight:labelHeight style:THLSingleLineTextFieldStyleNone];
}
...
- (instancetype) initWithCoder:(NSCoder *)coder
{
    ...
    if ( self )
    {
        _style = THLSingleLineTextFieldStyleNone;
...
- (void)addLineView {
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height + 10 , self.frame.size.width, 1)];
...
- (void) setStyle:(THLSingleLineTextFieldStyle)style
{
    _style = style;
    [self updateStyle];
}
...
- (void) propertyInit
{
    ...
    _placeholderInactiveColor = kTHLNUIGrayFontColor;
    if ([[UIDevice currentDevice].systemVersion floatValue] >= 7) {
        _placeholderActiveColor = kTHLNUIAccentColor;
    }
    _hintText = nil;
...
- (void) updateStyle
{
    switch ( self.style )
    {
        case THLSingleLineTextFieldStyleEmail:
            self.placeholder = @"Email";
            self.format = nil;
            self.validationBlock = ^NSDictionary *(THLSingleLineTextField *textField, NSString *text) {
            ...
            break;
        case THLSingleLineTextFieldStylePhone:
            self.placeholder = @"Phone";
            self.keyboardType = UIKeyboardTypePhonePad;
            self.format = @"(###)###-####";
            break;
        case THLSingleLineTextFieldStylePassword:
